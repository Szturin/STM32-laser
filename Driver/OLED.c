...
/**
  * @函	数		OLED显示波形
  * @参	数		t1时刻坐标点，t2时刻坐标点,轴线位置，起始波形绘制点，中止时间点
  * @返回值		无
  * @特殊说明		
  */
void OLED_DrawWave(int16_t *t,int16_t *y,int16_t *t_temp,int16_t *y_temp,int16_t center_axis,int16_t start_point,int16_t end_point)
{
	int16_t x1,y1;
	int16_t x2,y2;
	
	if(*t_temp != *t)//OLED波形连线 邻间断时刻 产生变化
	{
		/*t1时刻坐标点1*/
		x1=start_point-*t_temp;		
		y1=(*y_temp<0)?center_axis-*y_temp:center_axis+*y_temp;

		/*t2时刻坐标点2*/
		x2=start_point-*t;
		y2=(*y<0)?center_axis-*y:center_axis+*y;

		OLED_DrawLine(x1,y1,x2,y2);//绘制两点连线
		*y_temp=*y;
		*t_temp=*t;
	}
	
	if(*t>=end_point)//t2时刻大于结束时间点，将数组更新到OLED上
	{
		
		OLED_Update();//OLED更新
		*t=0;//清除t1,t2时刻
		*t_temp=0;
		OLED_Clear();//OLED清屏，防止不同波形像素重叠
	}
}

/**
  * @函	数		OLED波形绘制周期定义
  * @参	数		OLED单位间隔时间，OLED t2时间点
  * @返回值		
  * @特殊说明	调用此函数counter次，OLED 时间点才会发生偏移，即形成单位时间间隔的时间点，可以利用中断函数形成时间点
  */
void OLED_DrawWave_Cycle(int16_t *Wave_Time,int16_t *t,uint16_t counter)
{
	if(++*Wave_Time== counter)
	{
		*t+=2;
		*Wave_Time=0;
	}	
